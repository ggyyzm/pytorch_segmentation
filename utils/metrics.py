...
def batch_average_accuracy(output, target, num_class):
    _, predict = torch.max(output, 1)
    ...
    area_inter = torch.histc(intersection.float(), bins=num_class-1, max=num_class-1, min=1)
    area_lab = torch.histc(target.float(), bins=num_class-1, max=num_class-1, min=1)
    assert (area_inter <= area_lab).all(), "Intersection area should be smaller than Union area"
    return area_inter.cpu().numpy(), area_lab.cpu().numpy()


def eval_metrics(output, target, num_classes):
    correct, labeled = batch_pix_accuracy(output.data, target)
    inter, lab = batch_average_accuracy(output.data, target, num_classes)
    return [np.round(correct, 5), np.round(labeled, 5), np.round(inter, 5), np.round(lab, 5)]
...
def inter_over_union(output, target, num_class):
    ...
    area_pred, _ = np.histogram(output, bins=num_class-1, range=(1, num_class-1))
    area_lab, _ = np.histogram(target, bins=num_class-1, range=(1, num_class-1))
    area_union = area_pred + area_lab - area_inter
